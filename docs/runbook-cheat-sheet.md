...
1. Clone or fork the Nava Labs Assistive Chatbot template into a new repository on GitHub.
...
1. If any of your configured datasets require credentials (API tokens, space IDs, etc.), set them in your `.env` file and export them in your shell environment. Each dataset's ingestion script should document what it requires.
1. Run the refresh ingestion script: `./refresh-ingestion.sh all` (or pass a specific `dataset_id` instead of `all`).
...
See [Data Management](data-management.md) for details on adding new datasets and running ingestion steps individually.

## Deploying new versions of the chatbot

By default, commits to `main` automatically trigger a deployment to the `dev` environment. For the full deployment workflow — including cutting production releases and using the `Deploy App` GitHub Actions workflow — see [Deployments and Releases](releases.md).
